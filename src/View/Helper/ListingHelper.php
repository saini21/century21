...
class ListingHelper extends Helper {
        ...
        if (!empty($this->fields)) {
            ...
            <div class="faqs table-responsive g-mb-40">
                        ...
                        <?php if ($this->hasPagination) { ?>
                            ...
                            <th style="width: 4%;">
                                <div class="form-group g-mb-10">
                                    <label class="u-check g-pl-25" title="Select / Unselect All">
                                        <input class="<?= $inputClasses; ?>" type="checkbox" name="selectAll"
                                               id="selectAll"
                                               value="true">
                                    ...
                                    <th style="width: 4%;">
                                        <div class="form-group g-mb-10">
                                            <label class="u-check g-pl-25">
                                                <input class="<?= $inputClasses; ?> select-row" type="checkbox"
                                                       name="ids[]"
                                                       value="<?= $this->obj->id; ?>">
                                                <div class="<?= $labelClasses; ?>">
                                                    <i class="fa" data-check-icon=""></i>
                ...
                <?php if ($this->hasPagination) { ?>
                    ...
                    <div id="deleteConfirmModal" class="text-left g-color-white g-bg-gray-dark-v1 g-overflow-y-auto  g-pa-20"
                         style="display: none; width: auto; height: auto; padding: 10%;">
                        <button type="button" class="close" onclick="Custombox.modal.close();">
                            <i class="hs-icon hs-icon-close"></i>
                        </button>
                        <h4 class="h4 g-mb-20">Delete <?= Inflector::humanize(Inflector::singularize(Inflector::underscore($this->controller))) ?></h4>
    ...
    public function fieldValue() {
        if (strpos($this->field['name'], '_id') !== false) {
            ...
            if ($this->obj->has($relatedModel)) {
                if (!empty($this->field['related_model_fields'])) {
                    ...
                    $this->objectFieldValue = implode(" ", $values);
                    
                } else {
                    $this->objectFieldValue = $this->obj->{$relatedModel}->name;
                }
            } else {
                $this->objectFieldValue = "";
            }
        } else {
            $this->objectFieldValue = $this->obj->{$this->field['name']};
        }
    }
            ...
            <script>
                $(document).ready(function () {
                    ...
                    $('.active-deactive').click(function () {
                        ...
                        $.ajax({
                            url: SITE_URL + "helpers/changeStatus/",
                            type: "POST",
                            data: {model, field, id,},
                            dataType: "json",
                            beforeSend: function () {
        ...
        if ($this->hasPagination) {
            ...
            <div class="row g-mt-10">
                <div class="col-md-6">
                    ...
                    <?= $this->view->Form->create(null, ['id' => 'bulkActionsFrom', 'url' => ['controller' => 'Admins', 'action' => 'bulk', 'prefix' => 'admin']]) ?>
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group u-select--v3 g-pos-rel g-brd-gray-light-v7 g-rounded-4 mb-0">
                                <select name="action" class="<?= $classes; ?>" title="Bulk Actions"
                                        style="display: none;" data-live-search="true" id="BulkActoins">
                                    <?php foreach ($this->bulk['actions'] as $value => $label) { ?>
                                        <option value="<?= $value; ?>" data-content='<span class="d-flex align-items-center w-100"><span>
                            ...
                            <label for="BulkActoins" class="error"></label>
                        </div>
                        <input type="hidden" name="ids" value="" id="selectedIds"/>
                        <div class="col-md-6">
                            <button class="btn btn-primary btn-lg" id="applyAction">Apply</button>
                        </div>
                    </div>
